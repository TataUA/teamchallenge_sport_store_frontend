...
import {
  IFilters,
  IProductsPageInitialProps,
} from "@/app/products/[...sub_category]/page";
...
const getSortedAndFilteredProducts = async ({
  ...
  const products: IProduct[] = await fetchProductsAction(
    getTranslatedSubcategoryFromEnglishToUkraine(sub_category),
  );
  const filteredProductBySubcategory = products.filter(
    (product) =>
      getTranslatedSubcategoryFromUkraineToEnglish(
        product.category.sub_category,
      ) === sub_category.toLowerCase(),
  );

  const filteredProductByGender = filteredProductBySubcategory.filter(
    (product) => {
      if (filters.gender) {
        return (
          product.category.gender.toLowerCase() ===
...
const ProductsList = async (props: IProductsPageInitialProps) => {
  ...
  return (
    <section className="h-full w-full">
      <NavigationByCategoryAndGender {...propsToPass} />
      <div className="">
        <ProductsListMainContent {...propsToPass} />
      </div>
    </section>
  );
};

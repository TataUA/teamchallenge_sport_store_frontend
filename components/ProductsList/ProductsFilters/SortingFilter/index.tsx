      ...
      <ResponsiveModal isOpen={isSortingFilterOpen} onClose={() => setIsSortingFilterOpen(false)}>
        ...
        {sortingProductsFilers.map((item, index) => (
          <div key={item.id} className="mb-4 min-[2800px]:mb-8">
            <div className={cn("flex justify-between items-center px-4 [&>svg]:fill-blue [&>svg]:hidden [&>svg]:mb-2", {
              '[&>svg]:block': currentFilterValue === item.id.toLocaleLowerCase()
            })}>
              <Link 
                className={cn("text-base text-[#272728] mb-2 font-medium cursor-pointer hover:text-blue min-[2800px]:text-3xl", {
                  'text-blue': currentFilterValue === item.id.toLocaleLowerCase()
                })}
